#!/usr/bin/env python3
"""AEOS Lint — lightweight operational validation.
...
This script is configurable via environment variables:
- AEOS_GOVERNANCE_DIR: Directory containing governance files (default: .aegos/governance)
- AEOS_PROTOCOLS_DIR: Directory containing protocol files (default: .aegos/protocols)
- AEOS_TEMPLATES_DIR: Directory containing template files (default: .aegos/templates)
- AEOS_WORKFLOWS_DIR: Directory containing workflow files (default: .aegos/workflows)
- AEOS_MEMORY_DIR: Directory containing memory artifacts (default: memory)
- AEOS_AGENTS_DIR: Directory containing agent definitions (default: .opencode/agents)
"""
...
GOVERNANCE_DIR = os.environ.get("AEOS_GOVERNANCE_DIR", os.path.join(ROOT_DIR, ".aegos", "governance"))
PROTOCOLS_DIR = os.environ.get("AEOS_PROTOCOLS_DIR", os.path.join(ROOT_DIR, ".aegos", "protocols"))
TEMPLATES_DIR = os.environ.get("AEOS_TEMPLATES_DIR", os.path.join(ROOT_DIR, ".aegos", "templates"))
WORKFLOWS_DIR = os.environ.get("AEOS_WORKFLOWS_DIR", os.path.join(ROOT_DIR, ".aegos", "workflows"))
MEMORY_DIR = os.environ.get("AEOS_MEMORY_DIR", os.path.join(ROOT_DIR, "memory"))
AGENTS_DIR = os.environ.get("AEOS_AGENTS_DIR", os.path.join(ROOT_DIR, ".opencode", "agents"))
...
def collect_memory_artifacts(directory):
    ...
    artifacts = []
    mem_dir = os.path.join(MEMORY_DIR, directory)
...
def check_memory_directories():
    ...
    for dname in MEMORY_DIRS:
        if not check_dir(os.path.join(MEMORY_DIR, dname), f"memory/{dname}"):
            all_ok = False
    return all_ok
...
def check_agent_registry():
    print("\n[3] Agent Registry")
    reg_path = os.path.join(AGENTS_DIR, "agent_registry.yaml")
    if not os.path.isfile(reg_path):
        # Also check .opencode/opencode.json as alternative
        opencode_json = os.path.join(ROOT_DIR, ".opencode", "opencode.json")
        if os.path.isfile(opencode_json):
            print(f"  OK:   opencode.json found (agent registry via OpenCode config)")
            return True
        print(f"  FAIL: agent_registry.yaml or opencode.json missing")
        return False
...
def check_opencode_config():
    ...
    opencode_json = os.path.join(ROOT_DIR, ".opencode", "opencode.json")
    if not os.path.isfile(opencode_json):
        print(f"  WARN: .opencode/opencode.json not found (optional for AEOS)")
        return all_ok

    print(f"  OK:   .opencode/opencode.json found")

    if not HAS_YAML:
        print(f"  INFO: PyYAML not installed, skipping JSON validation")
        return all_ok
    ...
    try:
        ...
        if "agent" not in config:
            print(f"  WARN: No agents defined in opencode.json")
            all_ok = False
        else:
            agent_count = len(config["agent"])
            print(f"  OK:   {agent_count} agents defined")

        if "default_agent" not in config:
            print(f"  WARN: No default_agent specified")
        else:
            print(f"  OK:   default_agent: {config['default_agent']}")

    except json.JSONDecodeError as e:
        print(f"  FAIL: Invalid JSON in opencode.json: {e}")
        all_ok = False
...
def main():
    print("=" * 60)
    print("AEOS Lint — Operational Validation")
    print("=" * 60)
    print(f"\nConfiguration:")
    ...
    results.append(("OpenCode Configuration", check_opencode_config()))
