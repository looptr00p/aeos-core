...
All agents operating within this project MUST adhere to the following constraints.
...
All agents MUST be registered in `.opencode/opencode.json`.
...
All governance files are located in `.aegos/`:
- Governance rules: `.aegos/governance/`
- Protocols: `.aegos/protocols/`
- Workflows: `.aegos/workflows/`
- Templates: `.aegos/templates/`
- Memory: `memory/`

### State Graph

Workflows operate as a state graph with feedback loops, not as linear pipelines.
See `.aegos/workflows/state_graph.md` for the complete graph definition.
See `.aegos/protocols/STATE_TRANSITION_PROTOCOL.md` for transition recording rules.
