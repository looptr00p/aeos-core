...
def _check_v03_requirements(core_root: Path) -> list[str]:
    errors: list[str] = []

    for rel in REQUIRED_V03_FILES:
        if not (core_root / rel).is_file():
            errors.append(f"Missing v0.3 required file: aeos-core/{rel}")

    severity_path = core_root / "governance/GOVERNANCE_SEVERITY_MODEL.md"
    ...
    escalation_path = core_root / "templates/escalation_template.md"
    if escalation_path.is_file():
        text = escalation_path.read_text(encoding="utf-8")
        if "ESC-001" not in text:
            errors.append("Missing ESC-001 in escalation_template.md")

    return errors


def _check_experimental_lab_requirements(core_root: Path) -> list[str]:
    errors: list[str] = []
...
def main() -> int:
    ...
    errors = _check_v03_requirements(core_root)
    errors.extend(_check_experimental_lab_requirements(core_root))
    if errors:
